...
public class PatientBusinessTest {
  ...
  @Test
  void changeLocationInformationsCorrectlyUpdatePatientInfos() {
    try {
      patientBusiness.changeLocationInformations(
          john,
          "1410 avenue jean monnet",
          "Jardin");
    } catch (InvalidNameException | IllegalArgumentException e) {
      fail("Should not throw exeception");
    }
  }

  @Test
  void changeLocationInformationsThrowIllegalArgWhenAdressIsNotGiven() {
    final Exception eEmpty = assertThrows(IllegalArgumentException.class,
        () -> patientBusiness.changeLocationInformations(doe,
            "",
            "Jardin"));

    final Exception eNull = assertThrows(IllegalArgumentException.class,
        () -> patientBusiness.changeLocationInformations(doe,
            null,
            "Jardin"));

    final String actualMessageEmpty = eEmpty.getMessage();
    final String expectedMessageEmpty = "ArgumentChecker Failed Empty variable. ";

    final String actualMessageNull = eNull.getMessage();
    final String expectedMessageNull = "ArgumentChecker Failed Null variable. ";
  ...
  @Test
  void changeLocationInformationsThrowIllegalArgWhenCityIsNotGiven() {
    final Exception eEmpty = assertThrows(IllegalArgumentException.class,
        () -> patientBusiness.changeLocationInformations(doe,
            "1410 avenue jean monnet",
            ""));
  ...
  @Test
  void changeSurnameCorrectlyUpdatePatient() {
    try {
      patientBusiness.changeSurname(
          john, "Anderson");
    } catch (InvalidNameException | IllegalArgumentException e) {
      fail("Should not throw exeception");
    }
  ...
  @Test
  void changeSurnameThrowIllegalArgumentWhenGivenBadSurname() {
    final Exception eEmpty = assertThrows(IllegalArgumentException.class,
        () -> patientBusiness.changeSurname(
          john, ""));
    final Exception eNull = assertThrows(IllegalArgumentException.class,
